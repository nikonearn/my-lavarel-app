...
class StockController extends Controller
{
    ...
    {
        $page_title = __('Stock Holdings');
        $template = config('site.template');
        $search = $request->search;
        $conversion = rateConverter(1, 'USD', getSetting('currency'), 'stock');
        $exchange_rate = $conversion['exchange_rate'];

        $holdings_query = StockHolding::with('user')
        ...
        if ($request->has('export')) {
            ...
            if ($exportType == 'pdf') {
                ...
                $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView("templates.$template.blades.admin.pdf.stocks", [
                    'holdings' => $items,
                ...
                return $pdf->download('equity-inventory-' . now()->format('Y-m-d-H-i-s') . '.pdf');
            }

            if ($exportType == 'sql') {
                $headers = [
                    'Content-Type' => 'application/sql',
                    'Content-Disposition' => 'attachment; filename="stocks-dump-' . now()->format('Y-m-d-H-i-s') . '.sql"',
                ];
                $callback = function () use ($items) {
                    $file = fopen('php://output', 'w');
                    fwrite($file, "-- Stock Holdings Table Dump\n\n");
                    foreach ($items as $item) {
                        ...
                        fwrite($file, "INSERT INTO stock_holdings (" . implode(', ', $cols) . ") VALUES (" . implode(', ', $vals) . ");\n");
                    }
                    fclose($file);
                };
            ...
            if ($exportType == 'csv') {
                $headers = [
                    ...
                    "Content-Disposition" => "attachment; filename=stock-holdings-" . now()->format('Y-m-d-H-i-s') . ".csv",
                ];
        ...
        if ($request->ajax() && !$request->has('export')) {
            return view("templates.$template.blades.admin.stocks.index", compact('page_title', 'holdings', 'exchange_rate', 'stats'));
        }
    ...
    public function history(Request $request)
    {
        ...
        $history_query = StockHoldingHistory::with('user')
            ->when($search, function ($q) use ($search) {
                return $q->where('ticker', 'like', "%$search%")
                    ->orWhereHas('user', function ($uq) use ($search) {
                        $uq->where('username', 'like', "%$search%")
        ...
        if ($request->has('export')) {
            ...
            if ($exportType == 'pdf') {
                $orientation = count($selectedCols) <= 8 ? 'portrait' : 'landscape';
                $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView("templates.$template.blades.admin.pdf.stock-history", [
                    'history' => $items,
                    'page_title' => $page_title,
                    'columns' => $selectedCols,
                    'exchange_rate' => $exchange_rate,
                ...
                return $pdf->download('stock-order-history-' . now()->format('Y-m-d-H-i-s') . '.pdf');
            }

            if ($exportType == 'sql') {
                $headers = [
                    'Content-Type' => 'application/sql',
                    'Content-Disposition' => 'attachment; filename="stock-history-dump-' . now()->format('Y-m-d-H-i-s') . '.sql"',
                ];
                $callback = function () use ($items) {
                    $file = fopen('php://output', 'w');
                    fwrite($file, "-- Stock History Table Dump\n\n");
                    foreach ($items as $item) {
                        $attributes = is_object($item) && method_exists($item, 'getAttributes') ? $item->getAttributes() : (array) $item;
                        $cols = array_keys($attributes);
                        $vals = array_map(fn($v) => is_null($v) ? 'NULL' : "'" . addslashes((string) $v) . "'", array_values($attributes));
                        fwrite($file, "INSERT INTO stock_holding_histories (" . implode(', ', $cols) . ") VALUES (" . implode(', ', $vals) . ");\n");
                    }
                    fclose($file);
                };
                return response()->stream($callback, 200, $headers);
            ...
            if ($exportType == 'csv') {
                $headers = [
                    "Content-type" => "text/csv",
                    "Content-Disposition" => "attachment; filename=stock-order-history-" . now()->format('Y-m-d-H-i-s') . ".csv",
                ];
                $callback = function () use ($items, $selectedCols, $exchange_rate) {
                    $file = fopen('php://output', 'w');
                    fputcsv($file, array_values($selectedCols));
        ...
        $rawVolume = StockHoldingHistory::selectRaw('DATE(created_at) as day, transaction_type as type, SUM(amount) as total')
        ...
        // Ticker distribution
        $ticker_data = StockHoldingHistory::select('ticker', \DB::raw('COUNT(*) as count'), \DB::raw('SUM(amount) as total'))
            ->groupBy('ticker')
            ->orderByDesc('count')
            ->limit(10)
            ->get()
        ...
        if ($request->ajax() && !$request->has('export')) {
            return view("templates.$template.blades.admin.stocks.history", compact('page_title', 'history', 'exchange_rate', 'stats', 'graph_data', 'ticker_data'));
        }
    ...
    {
        $request->validate(['id' => 'required|exists:stock_holdings,id']);
        StockHolding::findOrFail($request->id)->delete();
    ...
    {
        $request->validate(['id' => 'required|exists:stock_holding_histories,id']);
        StockHoldingHistory::findOrFail($request->id)->delete();
