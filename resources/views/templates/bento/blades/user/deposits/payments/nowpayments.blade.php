...
@section('content')
    @php
        $currencies = json_decode($payment_method->payment_information, true);
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1">
                ...
                <p class="text-text-secondary mt-2 pl-5 border-l border-white/5 font-light">
                    {{ __('Select your preferred cryptocurrency to complete the deposit of') }}
                    <span
                        class="text-white font-bold">{{ getSetting('currency_symbol') }}{{ number_format($user_payment_details['amount'], 2) }}</span>
                </p>
            </div>
            ...
            <div class="lg:col-span-2 grid grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div
                    ...
                    <div class="flex items-baseline gap-1 mt-1 z-10">
                        <span class="text-xl font-bold text-white tracking-tight">{{ $payment_method->name }}</span>
                    </div>
                </div>
                <div
    ...
    <div id="confirm-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div
                class="bg-[#0f1115] border border-white/10 rounded-2xl w-full max-w-md transform scale-95 opacity-0 transition-all duration-300 modal-content relative overflow-hidden shadow-2xl">
                <div
                ...
                <div class="p-8">
                    ...
                    <div class="space-y-4">
                        @php
                            ...
                            $feePercent = getSetting('deposit_fee');
                            $feeAmount = ($amount * $feePercent) / 100;
                            $totalAmount = $amount + $feeAmount;
                        @endphp
                        <div class="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-3">
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-text-secondary">{{ __('Deposit Amount') }}</span>
                                <span
                                    class="text-white font-mono font-bold">{{ getSetting('currency_symbol') }}{{ number_format($amount, 2) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-text-secondary">{{ __('Processing Fee') }}
                                    ({{ $feePercent }}%)</span>
                                <span
                                    class="text-accent-primary font-mono font-bold">+{{ getSetting('currency_symbol') }}{{ number_format($feeAmount, 2) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-base border-t border-white/5 pt-3">
                                <span class="text-white font-bold">{{ __('Total to Pay') }}</span>
                                <span
                                    class="text-accent-primary font-mono font-bold underline decoration-accent-primary/30 underline-offset-4">
                                    {{ getSetting('currency_symbol') }}{{ number_format($totalAmount, 2) }}
                                </span>
                            </div>
                        ...
                        <form id="nowpayments-form" action="{{ route('user.deposits.new.nowpayments-validate') }}"
                            method="POST" class="ajax-form" data-action="redirect">
                            @csrf
                            <input type="hidden" name="payment_currency" id="selected-currency-code">
                        ...
                        <p class="text-[10px] text-text-secondary text-center opacity-60">
                            {{ __('Final exchange rate will be calculated at the time of deposit.') }}
                        </p>
                    </div>
                </div>
            </div>
