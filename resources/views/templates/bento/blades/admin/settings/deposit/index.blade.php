            ...
            <div class="max-w-3xl">
                <div class="flex flex-col gap-2 mb-12">
                    ...
                    <p class="text-slate-500 text-sm font-medium tracking-wide">
                        {{ __('Configure platform deposit limits, fees, and payment gateway infrastructure.') }}
                    </p>
                </div>

                {{-- Global Configuration Form --}}
                <form id="deposit-settings-form" action="{{ route('admin.settings.deposit.settings.update') }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="input-group has-symbol">
                                    ...
                                    <input type="number" name="min_deposit" value="{{ $settings['min_deposit'] ?? 1 }}"
                                        required class="flat-input">
                                </div>
                                <span class="text-[10px] text-slate-600 font-medium">
                                    {{ __('The lowest amount a user can deposit.') }}
                                </span>
                            </div>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="input-group has-symbol">
                                    <span class="currency-symbol">{{ getSetting('currency_symbol') }}</span>
                                    <input type="number" name="max_deposit" value="{{ $settings['max_deposit'] ?? 6000 }}"
                                        required class="flat-input">
                                </div>
                                <span class="text-[10px] text-slate-600 font-medium">
                                    {{ __('The highest amount a user can deposit per session.') }}
                                </span>
                            </div>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <input type="number" name="deposit_fee" value="{{ $settings['deposit_fee'] ?? 5 }}"
                                    step="0.01" required class="flat-input">
                                <span class="text-[10px] text-slate-600 font-medium">
                                    {{ __('System fee applied to every successful deposit.') }}
                                </span>
                            </div>

                            <div class="flex flex-col gap-3">
                                <label class="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                                    {{ __('Deposit Expiry (Hours)') }}
                                </label>
                                <input type="number" name="deposit_expires_at"
                                    value="{{ $settings['deposit_expires_at'] ?? 10 }}" required class="flat-input">
                                <span class="text-[10px] text-slate-600 font-medium">
                                    {{ __('Time before an unconfirmed deposit is automatically canceled.') }}
                                </span>
                            </div>
                        </div>
                ...
                <div class="space-y-12 pb-24">
                    <div class="settings-section">
                        ...
                        <div id="content-manual" class="tab-content transition-all duration-300 flex flex-col gap-4">
                            @foreach ($manual_gateways as $gateway)
                                <div class="gateway-row" id="gateway-row-{{ $gateway->id }}">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('admin.settings.deposit.edit', $gateway->id) }}"
                            ...
                            <a href="{{ route('admin.settings.deposit.create') }}"
                        ...
                        <div id="content-automatic"
                            class="tab-content hidden transition-all duration-300 flex flex-col gap-6">
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
             ...
             * Form Submission
             */
            $('#deposit-settings-form').on('submit', function(e) {
                e.preventDefault();
            ...
            $(document).on('change', '.status-toggle', function() {
                ...
                $.ajax({
                    url: '{{ route('admin.settings.deposit.toggle-status') }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: id
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            ...
            $.ajax({
                url: `{{ url('admin/settings/deposit/delete') }}/${gatewayIdToDelete}`,
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
