...
@section('content')
    <div id="margin-orders-wrapper" class="space-y-8">
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-3 mb-5">
                        <div>
                            ...
                            <p class="text-[9px] text-slate-500 uppercase tracking-widest font-mono mt-0.5">
                                {{ __('Margin Orders Workflow') }} · {{ __('Execution Count') }}
                            </p>
                        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-black text-white tracking-tight">{{ __('Margin Orders') }}</h2>
                    <p class="text-[10px] text-text-secondary font-bold uppercase tracking-widest mt-1 opacity-50">
                        {{ __('Manage and monitor margin limit and market orders') }}</p>
                </div>
            </div>

            <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto">
                <form id="filter-form" method="GET" action="{{ route('admin.margin-trading.orders.index') }}"
                                ...
                                <tr class="group hover:bg-white/[0.01] transition-colors">
                                    ...
                                    <td class="px-8 py-5 text-right font-mono">
                                        ...
                                        <div class="text-[9px] text-text-secondary font-bold uppercase opacity-40">
                                            {{ number_format($order->size, 8) }} {{ __('Amount') }}</div>
                                    </td>
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @endif
                                            <button onclick="deleteOrder({{ $order->id }}, '{{ $order->ticker }}')"
                                ...
                                <tr>
                                    <td colspan="5" class="px-8 py-20 text-center font-body">
                                        ...
                                        <p class="text-text-secondary text-xs mt-1">
                                            {{ __('No margin orders found matching your search criteria.') }}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
@push('scripts')
    ...
    <script>
        ...
        function loadOrders(url) {
            ...
            $('#margin-orders-wrapper').addClass('opacity-50 pointer-events-none');

            $.get(url, function(data) {
                const $html = $(data);
                $('#margin-orders-wrapper').html($html.find('#margin-orders-wrapper').html());
                ...
                $('#margin-orders-wrapper').removeClass('opacity-50 pointer-events-none');
            });
        }
        ...
        window.cancelOrder = function(id, ticker) {
            confirmAction({
                ...
                if (result.isConfirmed) {
                    $.post("{{ route('admin.margin-trading.orders.cancel') }}", {
                        id: id,
                        _token: "{{ csrf_token() }}"
                    }).done(res => {
                        toastNotification(res.message, res.status);
                        if (res.status === 'success') loadOrders(window.location.href);
                    }).fail(() => toastNotification('{{ __('Request failed') }}', 'error'));
                }
            });
        }

        window.deleteOrder = function(id, ticker) {
            confirmAction({
                title: '{{ __('Delete Record?') }}',
                text: "{{ __('Are you sure you want to delete the order record for') }} " + ticker + "?",
                icon: 'error',
                confirmText: '{{ __('Delete Now') }}',
                isDanger: true
            }).then((result) => {
                if (result.isConfirmed) {
                    $.post("{{ route('admin.margin-trading.orders.delete') }}", {
                        id: id,
                        _token: "{{ csrf_token() }}"
                    }).done(res => {
                        toastNotification(res.message, res.status);
