                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p
                                    ...
                                    {{ __('Complete log of all stock purchase and liquidation events') }}</p>
                            </div>
                        </div>

                        <form id="filter-form" method="GET" action="{{ route('admin.stocks.history') }}"
                            class="flex-1 max-w-4xl flex flex-wrap gap-3 lg:justify-end">
                            @if (request('search') || request('type'))
                                <button type="button" data-url="{{ route('admin.stocks.history') }}"
                                ...
                                <tr>
                                    <td colspan="8" class="px-8 py-20 text-center">
                                        ...
                                        <p class="text-text-secondary text-xs mt-1">
                                            {{ __('No stock transactions have been recorded yet.') }}</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        (function() {
            ...
            $(document).on('click', '#confirm-delete-btn', function() {
                ...
                $.ajax({
                    url: "{{ route('admin.stocks.history.delete') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: deleteId
                            ...
                            <div class="grid grid-cols-2 gap-3" id="export-columns-container">
                                ...
                                @foreach ($columns as $key => $col)
                                    <label
                                        ...
                                        <span
                                            class="text-sm font-medium text-slate-300 group-hover:text-white transition-colors">{{ __($col['label']) }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
