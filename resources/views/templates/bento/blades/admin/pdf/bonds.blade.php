    ...
    <table>
        <thead>
            <tr>
                @foreach ($columns as $key => $label)
                    <th class="{{ in_array($key, ['amount', 'coupon', 'interest_amount']) ? 'text-right' : '' }}">
                        {{ __($label) }}
                    </th>
                @endforeach
            </tr>
        ...
        <tbody>
            @forelse($holdings as $item)
                <tr>
                    @foreach ($columns as $key => $label)
                        <td class="{{ in_array($key, ['amount', 'coupon', 'interest_amount']) ? 'text-right' : '' }}">
                            @if ($key == 'username')
                                <span style="font-weight: 600;">{{ $item->user->username }}</span>
                            @elseif($key == 'amount' || $key == 'interest_amount')
                                <span class="font-mono">{{ showAmount($item->$key) }}</span>
                            @elseif($key == 'coupon')
                                <span class="font-mono">{{ number_format($item->coupon, 2) }}%</span>
                            @elseif($key == 'maturity_date')
                                {{ $item->maturity_date->format('M d, Y') }}
                            @elseif($key == 'status')
            ...
            @empty
                <tr>
                    <td colspan="{{ count($columns) }}" class="text-center" style="padding: 30px; color: #94a3b8;">
                        {{ __('No bond holdings found matching your criteria.') }}
                    </td>
                </tr>
            @endforelse
        </tbody>
