    ...
    <div class="header">
        ...
        <div class="report-info">
            ...
            <div class="report-meta">
                ...
                {{ __('Total Transactions') }}: {{ $transactions->count() }}
            </div>
        </div>
        ...
        <thead>
            <tr>
                @foreach ($columns as $key => $label)
                    <th
                        class="{{ in_array($key, ['amount', 'new_balance']) ? 'text-right' : (in_array($key, ['status', 'type']) ? 'text-center' : '') }}">
                        {{ __($label) }}
                    </th>
                @endforeach
            </tr>
                ...
                <tr>
                    @foreach ($columns as $key => $label)
                        <td
                            class="{{ in_array($key, ['amount', 'new_balance']) ? 'text-right' : (in_array($key, ['status', 'type']) ? 'text-center' : '') }}">
                            @if ($key == 'username')
                                <span style="font-weight: 600;">{{ $transaction->user->username }}</span>
                            @elseif($key == 'status' || $key == 'type')
                                <span class="status-badge status-{{ $transaction->$key }}">
                                    {{ ucfirst($transaction->$key) }}
                                </span>
                            @elseif(in_array($key, ['amount', 'new_balance']))
                                <span style="font-family: monospace;">{{ showAmount($transaction->$key) }}</span>
