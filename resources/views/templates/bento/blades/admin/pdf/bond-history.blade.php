    ...
    <div class="header">
        ...
        <div class="report-info">
            ...
            <div class="report-meta">
                ...
                {{ __('Total Entries') }}: {{ $history->count() }}
            </div>
        </div>
    ...
    <table>
        <thead>
            <tr>
                @foreach ($columns as $key => $label)
                    <th class="{{ $key == 'amount' ? 'text-right' : '' }}">
                        {{ __($label) }}
                    </th>
                @endforeach
            </tr>
        ...
        <tbody>
            @forelse($history as $item)
                <tr>
                    @foreach ($columns as $key => $label)
                        <td class="{{ $key == 'amount' ? 'text-right' : '' }}">
                            @if ($key == 'username')
                                <span style="font-weight: 600;">{{ $item->user->username }}</span>
                            @elseif($key == 'amount')
                                <span class="font-mono">{{ showAmount($item->amount) }}</span>
                            @elseif($key == 'transaction_type')
                                <span class="type-badge type-{{ $item->transaction_type }}">
