...
@section('content')
    <div class="space-y-8">
                ...
                <div class="relative z-10 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-text-secondary text-[10px] uppercase font-bold tracking-wider mb-0.5">
                            {{ __('Total Deposited') }}</p>
                        <h4 class="text-xl font-bold text-white">{{ showAmount($stats['total_deposited']) }}</h4>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div
                ...
                <div class="relative z-10 p-6 h-full flex flex-col">
                    <div class="mb-5">
                        <div class="text-sm font-bold text-white tracking-wide">{{ __('Deposit Status Distribution') }}
                        </div>
                        <p class="text-[9px] text-slate-500 uppercase tracking-widest font-mono mt-0.5">
                            {{ __('Amount by Status') }}
                        </p>
        ...
        <div class="bg-secondary border border-white/5 rounded-2xl overflow-hidden relative" id="deposits-wrapper">
            ...
            <div id="loading-spinner"
                ...
                <p class="mt-3 text-text-secondary font-medium">{{ __('Loading deposits...') }}</p>
            </div>
                ...
                <form id="filter-form" action="{{ route('admin.deposits.index') }}" method="GET"
                    ...
                    <div class="relative custom-dropdown" id="status-filter-dropdown">
                        ...
                        <div
                            ...
                            <div class="dropdown-option px-3 py-2 text-sm text-text-secondary hover:text-white hover:bg-white/5 cursor-pointer transition-colors"
                                data-value="partial_payment">{{ __('Partial Payment') }}</div>
                            <div class="dropdown-option px-3 py-2 text-sm text-text-secondary hover:text-white hover:bg-white/5 cursor-pointer transition-colors"
                            ...
                            <tr class="hover:bg-white/[0.01] transition-all group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <div>
                                            <a href="{{ route('admin.users.detail', $deposit->user->id) }}"
                                                class="text-xs text-white font-bold hover:text-accent-primary transition-colors block cursor-pointer">{{ $deposit->user->username }}</a>
                                            <span
                                                class="text-[10px] text-text-secondary block mb-1.5">{{ $deposit->user->email }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs text-white font-medium block">{{ $deposit->paymentMethod ? $deposit->paymentMethod->name : 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs text-slate-300 font-mono tracking-wider bg-white/5 px-2 py-1 rounded">{{ $deposit->transaction_reference }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-xs text-white font-bold">{{ showAmount($deposit->amount) }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs text-red-400 font-bold">-{{ showAmount($deposit->fee_amount) }}</span>
                                    <span
                                        class="text-[9px] text-text-secondary block mt-0.5">({{ $deposit->fee_percent }}%)</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs text-emerald-400 font-bold">{{ showAmount($deposit->total_amount) }}</span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @elseif($deposit->status == 'partial_payment')
                                        <span
                                            class="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 text-[10px] font-black uppercase tracking-tighter">{{ __('Partial Payment') }}</span>
                                    ...
                                    @else
                                        <span
                                            class="px-2 py-0.5 rounded-full bg-slate-500/10 text-slate-500 text-[10px] font-black uppercase tracking-tighter">{{ $deposit->status }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="button"
                                            class="edit-deposit-btn p-2 bg-accent-primary/10 text-accent-primary rounded-lg hover:bg-accent-primary/20 transition-all cursor-pointer"
                                        ...
                                        <button type="button"
                                            class="delete-deposit-btn p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-all cursor-pointer"
                                            data-id="{{ $deposit->id }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                ...
                <div class="relative z-10 text-center">
                    ...
                    <p class="text-sm text-slate-400 mb-8 px-4">
                        {{ __('Are you sure you want to delete this deposit? This action cannot be undone.') }}
                    </p>
                            ...
                            <div class="grid grid-cols-2 gap-3" id="export-columns-container">
                                @php
                                    $columns = [
                                        ...
                                        'payment_method_name' => ['label' => 'Gateway', 'default' => true],
                                        'amount' => ['label' => 'Amount', 'default' => true],
                                        'fee_amount' => ['label' => 'Fee', 'default' => true],
                                        'total_amount' => ['label' => 'Total', 'default' => true],
        ...
        $(document).ready(function() {
            ...
            $('#confirm-export').on('click', function() {
                ...
                if (selectedCols.length === 0) {
                    toastNotification('{{ __('Please select at least one column to export.') }}',
                        'error');
                    return;
                }
            ...
            function loadTable(url) {
                ...
                $('#deposits-wrapper .overflow-x-auto').css('opacity', '0.5');

                $.ajax({
                    url: url,
                    type: 'GET',
                    success: function(data) {
                        $('#deposits-wrapper').html($(data).find('#deposits-wrapper').html());
                    ...
                    success: function(res) {
                        if (res.success || res.status === 'success') {
                            ...
                            toastNotification(res.message ||
                                'Deposit status updated successfully', 'success');
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            ...
                            toastNotification(res.message || 'Deposit deleted successfully',
                                'success');
