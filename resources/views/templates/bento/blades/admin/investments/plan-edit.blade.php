...
@section('content')
    <div class="space-y-8">
        ...
        @if (moduleEnabled('investment_module'))
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p class="text-sm text-text-secondary mt-1">
                        {{ __('Modify the configuration of this investment plan.') }}
                    </p>
                </div>
                <button type="button"
            ...
            <form id="editPlanForm" action="{{ route('admin.investments.plans.update', $plan->id) }}" method="POST">
                @csrf
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea id="description" name="description" rows="3" required
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-primary focus:ring-1 focus:ring-accent-primary transition-all placeholder:text-text-secondary/50"
                                        placeholder="{{ __('Briefly describe the investment strategy and focus.') }}">{{ $plan->description }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="number" id="min_investment" name="min_investment"
                                        value="{{ $plan->min_investment }}" step="any" required
                                ...
                                <div>
                                    <label for="max_investment"
                                        ...
                                        ({{ getSetting('currency') }}) <span class="text-red-500">*</span></label>
                                    <input type="number" id="max_investment" name="max_investment"
                                        value="{{ $plan->max_investment }}" step="any" required
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-primary focus:ring-1 focus:ring-accent-primary transition-all"
                            ...
                            <div class="mb-4">
                                <div>
                                    <label for="return_percent"
                                        ...
                                        (%) <span class="text-red-500">*</span></label>
                                    <input type="number" id="return_percent" name="return_percent"
                                        value="{{ $plan->return_percent }}" step="any" required
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label for="duration"
                                        ...
                                        <span class="text-red-500">*</span></label>
                                    <input type="number" id="duration" name="duration" value="{{ $plan->duration }}"
                                        min="1" required
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-primary focus:ring-1 focus:ring-accent-primary transition-all"
                                        placeholder="6">
                                </div>
                                <div>
                                    ...
                                    <select id="duration_type" name="duration_type" required
                                        ...
                                        <option value="hours" class="bg-slate-800 text-white"
                                            {{ $plan->duration_type == 'hours' ? 'selected' : '' }}>{{ __('Hours') }}
                                        </option>
                                        <option value="days" class="bg-slate-800 text-white"
                                            {{ $plan->duration_type == 'days' ? 'selected' : '' }}>{{ __('Days') }}
                                        </option>
                                        <option value="months" class="bg-slate-800 text-white"
                                            {{ $plan->duration_type == 'months' ? 'selected' : '' }}>{{ __('Months') }}
                                        </option>
                                        <option value="years" class="bg-slate-800 text-white"
                                            {{ $plan->duration_type == 'years' ? 'selected' : '' }}>{{ __('Years') }}
                                        </option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select id="return_interval" name="return_interval" required
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-primary transition-all appearance-none cursor-pointer">
                                        <option value="hourly" class="bg-slate-800 text-white"
                                            {{ $plan->return_interval == 'hourly' ? 'selected' : '' }}>{{ __('Hourly') }}
                                        </option>
                                        <option value="daily" class="bg-slate-800 text-white"
                                            {{ $plan->return_interval == 'daily' ? 'selected' : '' }}>{{ __('Daily') }}
                                        </option>
                                        <option value="weekly" class="bg-slate-800 text-white"
                                            {{ $plan->return_interval == 'weekly' ? 'selected' : '' }}>{{ __('Weekly') }}
                                        </option>
                                        <option value="monthly" class="bg-slate-800 text-white"
                                            {{ $plan->return_interval == 'monthly' ? 'selected' : '' }}>
                                            {{ __('Monthly') }}
                                        </option>
                                        <option value="yearly" class="bg-slate-800 text-white"
                                            {{ $plan->return_interval == 'yearly' ? 'selected' : '' }}>{{ __('Yearly') }}
                                        </option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select id="risk_profile" name="risk_profile" required
                                        ...
                                        <option value="conservative" class="bg-slate-800 text-white"
                                            {{ $plan->risk_profile == 'conservative' ? 'selected' : '' }}>
                                            {{ __('Conservative') }}
                                        </option>
                                        <option value="balanced" class="bg-slate-800 text-white"
                                            {{ $plan->risk_profile == 'balanced' ? 'selected' : '' }}>{{ __('Balanced') }}
                                        </option>
                                        <option value="growth" class="bg-slate-800 text-white"
                                            {{ $plan->risk_profile == 'growth' ? 'selected' : '' }}>{{ __('Growth') }}
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select id="investment_goal" name="investment_goal" required
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-primary transition-all appearance-none cursor-pointer">
                                        <option value="short_term" class="bg-slate-800 text-white"
                                            {{ $plan->investment_goal == 'short_term' ? 'selected' : '' }}>
                                            {{ __('Short Term') }}</option>
                                        <option value="medium_term" class="bg-slate-800 text-white"
                                            {{ $plan->investment_goal == 'medium_term' ? 'selected' : '' }}>
                                            {{ __('Medium Term') }}</option>
                                        <option value="long_term" class="bg-slate-800 text-white"
                                            {{ $plan->investment_goal == 'long_term' ? 'selected' : '' }}>
                                            {{ __('Long Term') }}</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div
                                        ...
                                        @php
                                            ...
                                            $planInterests = is_array($plan->interests)
                                                ? $plan->interests
                                                : ($plan->interests
                                                    ? json_decode($plan->interests, true)
                                                    : []);
                                        @endphp
                                            ...
                                            <label
                                                ...
                                                <div class="relative flex items-center">
                                                    <input type="checkbox" name="interests[]"
                                                        ...
                                                        {{ in_array($key, $planInterests) ? 'checked' : '' }}>
                                                    <div
                                ...
                                <label
                                    ...
                                    <div class="relative">
                                        <input type="checkbox" name="capital_returned" value="1"
                                            class="peer sr-only" {{ $plan->capital_returned ? 'checked' : '' }}>
                                        <div
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    toastNotification(xhr.responseJSON?.message ||
                        '{{ __('An error occurred while updating the plan.') }}', 'error');
                }
            });
        });
    </script>
