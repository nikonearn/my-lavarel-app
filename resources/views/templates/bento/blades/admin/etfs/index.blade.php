...
@section('content')
    <div id="etfs-content" class="space-y-8">
        ...
        @if (!moduleEnabled('etf_module'))
            <div
                ...
                <p class="text-[10px] text-slate-500 max-w-[200px] mb-6 font-medium leading-relaxed italic">
                    {{ __('ETFs module is disabled. Please enable the ETF module in settings to initialize.') }}
                </p>
                ...
                <div class="bg-secondary/30 border-b border-white/5 p-4 lg:p-6 relative">
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="text-white font-bold tracking-tight">{{ __('ETF Holdings') }}</h4>
                                <p
                        ...
                        <form id="filter-form" method="GET" action="{{ route('admin.etfs.index') }}"
                            ...
                            @if (request('search'))
                                <button type="button" data-url="{{ route('admin.etfs.index') }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-8 py-20 text-center">
                                        ...
                                        <h5 class="text-white font-bold">{{ __('No ETF Holdings') }}</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadTable(url = null) {
                ...
                $('#etfs-wrapper').css('opacity', '0.5');

                $.ajax({
                    url: requestUrl,
                    type: 'GET',
                    success: function(data) {
                        $('#etfs-content').html($(data).find('#etfs-content').html());
